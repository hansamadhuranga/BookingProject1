class SearchResultsPage {
    ...
    filterByRating(rating) {
        ...
        cy.get(`input[data-filters-item="class:class=${rating}"]`).click();
    }
    ...
    getSecondItemDetails() {
        cy.get('div[data-testid="property-card"]').eq(1).within(() => {
            cy.get('div[data-testid="title"]').invoke('text').as('hotelName');
            cy.get('span[data-testid="price-and-discounted-price"]').invoke('text').as('hotelPrice');
        });
    }

    clickSecondItem() {
        cy.get('div[data-testid="property-card"]').eq(1).click();
    }
}
